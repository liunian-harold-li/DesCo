...
class LvisEvaluatorFixedAP(object):
    ...
    def _summarize_fixed(self):
        ...
        try:
            ...
            id2name = {}
            for x in self.gt.dataset["categories"]:
                if ":" in x["name"]:
                    part_id_to_obj_part_ids[x["name"].split(":")[-1]].append(x["id"])
                else:
                    obj_cat_ids.append(x["id"])
                    id2name[x['id']] = x['name']
            sorted_cats = sorted(self.gt.dataset["categories"], key=lambda x: x["id"])
            ...
            for obj_cat in obj_cat_ids:
                ...
                obj_results_per_class[id2name[obj_cat]] = ap * 100
            ...
            print("no part evaluation")
        print("copypaste: %s,%s", ",".join(map(str, metrics.keys())), "path")
        return scores, results_processed
