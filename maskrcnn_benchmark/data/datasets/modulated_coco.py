...
from .od_to_grounding import convert_od_to_grounding_simple, check_for_positive_overflow, sanity_check_target_after_processing, convert_object_detection_to_grounding_optimized_for_od, od_to_grounding_optimized_streamlined
...
class CocoGrounding(torchvision.datasets.CocoDetection):
    ...
    def __getitem__(self, idx):
        ...
        tgt = [obj for obj in tgt if "iscrowd" not in obj or obj["iscrowd"] == 0]
        ...
        elif self.od_to_grounding_version != "legacy": # new and more streamlined version of od to grounding conversion
            # NOTE: target is rewritten here
        ...
        elif self.special_safeguard_for_coco_grounding:
            # Intended for LVIS
...
class ModulatedDataset(torchvision.datasets.CocoDetection):
    ...
    def __getitem__(self, idx):
        ...
        if self.caption_augmentation_version is not None:
            caption, target, spans = self.caption_augmentation(caption, target, gpt3_outputs = coco_img.get("gpt3_outputs", None))
        else:
            spans = None
...
def create_default_span(tokenized, positive_map, captions):
    '''
    This is used when we don't need the span; we need to create fake spans, where each span is just a subword
    '''
    # desired: each token now is a span
    token_length = tokenized.input_ids.shape[1]
    spans = []
    for i in range(token_length):
        print(tokenized.token_to_chars(i))
    pass

def transfer_token_mapping_to_span_mapping(all_spans, spans_positive):
    ...
    for spans in all_spans:
        all_spans_new += spans
    all_spans = all_spans_new

    index_spans = {}
    for i, span in enumerate(all_spans):
        index_spans[tuple(span)] = i

    positive_map = torch.zeros((len(spans_positive), len(all_spans)), dtype=torch.float)
    for box_i, spans_box_i in enumerate(spans_positive):
        for span in spans_box_i:
            positive_map[box_i, index_spans[tuple(span)]] = 1
